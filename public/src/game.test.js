...
function gameLoop() {
    ...
    for (let i = monsters.length - 1; i >= 0; i--) {
        const monster = monsters[i];
        if (monster.hp > 0) {
            const isDestroyed = false;//monster.move(base);
            if (isDestroyed) {
                /* 게임 오버 */
                // alert("게임 오버. 스파르타 본부를 지키지 못했다...ㅠㅠ");
                showAlert(
                    'GameOver',
                    '게임 오버. 스파르타 본부를 지키지 못했다...ㅠㅠ'
                );
                // 게임 종료 시나리오
                //location.reload();
            }
...
export function updateMonsterPosition(data) {
    console.log("updateMonsterPosition: " + data);
    // 여기서 lerp
    const { monsterId, x, y } = data;
    console.log("x, y: " + x + " , " + y);
    // 해당 몬스터 객체를 찾기
    let monster = monsters[monsterId];
    if (monster) {
        // lerp를 통해 몬스터의 위치를 부드럽게 갱신
        monster.moveWithLerp(x, y);
    }
}
...
function initGame() {
    ...
    // #region initGame Rendering
    initMap();
    // #endregion
}

// 서버와 클라이언트가 동시에 게임 시작
initGame();
sendEvent(2001);
